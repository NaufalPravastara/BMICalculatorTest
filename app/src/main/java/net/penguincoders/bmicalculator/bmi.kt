...
class bmi : AppCompatActivity() {
    ...
    private fun bmiStatusValue(bmi: Double): String {
        ...
        if (bmi < 18.5)
            bmiStatus = "Underweight, Use Some Healthy Fat"
        else if (bmi >= 18.5 && bmi < 25)
            bmiStatus = "Normal, You are Healthy"
        else if (bmi >= 25 && bmi < 30)
            bmiStatus = "Overweight, do some Exercise"
        else if (bmi > 30)
            bmiStatus = "Obese, do some routine Exercise"
        return bmiStatus
    }
}